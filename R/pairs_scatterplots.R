  ...
  if (all_pairs == FALSE) {
    for (i in seq_along(x)) {
      for (k in seq_along(y)) {
        ...
        if (corr.p < .001) {
          stats <-
            paste("r(", corr.n, ") = ", corr.r, ", ", "p < .001", sep = "")
        } else if (corr.p < .01) {
          stats <-
            paste("r(", corr.n, ") = ", corr.r, ", ", "p < .01", sep = "")
        } else if (corr.p < .05) {
          stats <-
            paste("r(", corr.n, ") = ", corr.r, ", ", "p < .05", sep = "")
        } else if (corr.p >= .05) {
          stats <-
            paste("r(", corr.n, ") = ", corr.r, ", ", "p > .05", sep = "")
        }
        ...
        p <- ggplot2::ggplot(data, ggplot2::aes(x = get(x[i]), y = get(y[k]))) +
          ...
          ggplot2::annotate("text", Inf, -Inf, label = stats,
                            hjust = .5, vjust = 4, size = 5) +
    ...
    for (i in seq_along(variable.list)) {
      if (i != length(variable.list)) {
        for (k in (i + 1):length(variable.list)) {
          ...
          # Calculate correlation statistics
          corr <- stats::cor.test(data[[variable.list[i]]],
                                  data[[variable.list[k]]])
          corr.r <- round(corr$estimate[[1]], digits = 2)
          corr.p <- corr$p.value
          corr.n <- corr$parameter[[1]] + 2
          # Create text of statistics to add to plot
          ...
          # Create main scatter plot. Add text annotation
          p <- ggplot2::ggplot(data, ggplot2::aes(x = get(variable.list[i]),
                                                  y = get(variable.list[k]))) +
